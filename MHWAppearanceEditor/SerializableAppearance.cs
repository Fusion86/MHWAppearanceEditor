    ...
    public class SerializableAppearance
    {
        ...
        public void ApplyToSaveSlot(SaveSlot saveSlot)
        {
            ...
            foreach (var prop in thisType.GetProperties())
            {
                ...
                if (saveSlotProp != null)
                {
                    // This is the only thing that is different between this method and the constructor for this class, the destination where to __set__ the value
                    saveSlotProp.SetValue(saveSlot, prop.GetValue(this));
                }
            }
        }
